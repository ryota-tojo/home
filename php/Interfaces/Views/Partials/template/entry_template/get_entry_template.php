...
function isExistEntryTemplate($groups_id = null, $template_id = null){
    $result = apiCallEntryTemplateRefer($groups_id, $template_id);
...
function getEntryTemplate($groups_id,$template_id)
{

    if($groups_id == null){
        $msg = "Exception - " . UI_ITEM_ENTRY_TEMPLATE . "取得：" . UI_ITEM_GROUPS_ID . "がnullです";
        createLogs(LOG_TYPE_ERROR, $msg);
        throw new Exception($msg);
    }
    if($template_id == null){
        $msg = "Exception - " . UI_ITEM_ENTRY_TEMPLATE . "取得：" . UI_ITEM_ENTRY_TEMPLATE . "IDがnullです";
        createLogs(LOG_TYPE_ERROR, $msg);
        throw new Exception($msg);
    }
    ...
    if ($result['status'] == "error") {
        $msg = UI_ITEM_ENTRY_TEMPLATE . "取得：" . UI_ITEM_ENTRY_TEMPLATE . "情報の取得に失敗しました";
        createLogs(LOG_TYPE_ERROR, $msg);
        return [
...
function getEntryTemplateList($groups_id)
{
    ...
    $result = apiCallEntryTemplateRefer($groups_id, null);
    if ($result['status'] == "error") {
        $msg = UI_ITEM_ENTRY_TEMPLATE . "取得：" . UI_ITEM_ENTRY_TEMPLATE . "情報の取得に失敗しました";
        createLogs(LOG_TYPE_ERROR, $msg);
        return [];
...
function getEntryTemplateListForActive($groups_id)
{
...
function getEntryTemplateCount($groups_id)
{
    $cnt = 0;
    $result = apiCallEntryTemplateRefer($groups_id, null);
...
function getMaxEntryTemplateNo($groups_id)
{
    $template_max_no=0;
    $result = apiCallEntryTemplateRefer($groups_id);
    if ($result['status'] == "error") {
        return 0;
    }
    $datas = $result['data']['template_list'];
