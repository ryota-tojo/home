...
function autoActivateForInputTemplate($groups_id)
{
    $cnt = 0;
    $template_list = getInputTemplateList($groups_id);
    foreach ($template_list as $template) {
        if ($template['use'] == 1 and $template['deleted'] == 1) {
            ...
            if ($active_flag1 == True and $active_flag2 == True) {
                apiCallInputTemplateUnDisable($groups_id, $template['template_id']);
                $cnt++;
            }
        }
    }
...
function autoDeactivateForInputTemplate($groups_id)
{
    $cnt = 0;
    $template_list = getInputTemplateList($groups_id);
    foreach ($template_list as $template) {
        if ($template['deleted'] == 0) {
            ...
            if ($de_active_flag1 == True or $de_active_flag2 == True) {
                apiCallInputTemplateDisable($groups_id, $template['template_id']);
                $cnt++;
            }
        }
    }
