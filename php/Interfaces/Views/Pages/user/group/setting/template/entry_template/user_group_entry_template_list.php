...
if (isset($_POST['change_btn'])) {
    ...
    if (isset($_POST['selected_datas'])) {
        ...
        $result_data = shoppingDataPostActionForEntryTemplateInputEvent($_SESSION['user_groups_id'],$_POST['selected_datas']);
...
if (isset($_POST['use_btn'])) {
    ...
    if (isset($_POST['selected_datas'])) {
        $result_data = entryTemplatesPostActionForUsageEvent($_SESSION['user_groups_id'],$_POST['selected_datas']);
        $result = json_decode($result_data,True);
        if($result['status'] == 'error'){
            $entry_error = true;
        }
        ...
        $message = UI_ITEM_ENTRY_TEMPLATE . "が選択されていません";
        $entry_error = true;
        createLogs(LOG_TYPE_ERROR, UI_ITEM_ENTRY_TEMPLATE . "使用化 - " . UI_ITEM_ENTRY_TEMPLATE . "未選択");
    }
}
...
if (isset($_POST['un_use_btn'])) {
    ...
    if (isset($_POST['selected_datas'])) {
        $result_data = entryTemplatesPostActionForUnusageEvent($_SESSION['user_groups_id'],$_POST['selected_datas']);
        $result = json_decode($result_data,True);
        if($result['status'] == 'error'){
            $entry_error = true;
        }
        ...
        $message = UI_ITEM_ENTRY_TEMPLATE . "が選択されていません";
        $entry_error = true;
        createLogs(LOG_TYPE_ERROR, UI_ITEM_ENTRY_TEMPLATE . "不使用化 - " . UI_ITEM_ENTRY_TEMPLATE . "未選択");
    }
}
...
if (isset($_POST['on_btn'])) {
    ...
    if (isset($_POST['selected_datas'])) {
        $result_data = entryTemplatesPostActionForActivateEvent($_SESSION['user_groups_id'],$_POST['selected_datas']);
        $result = json_decode($result_data,True);
        if($result['status'] == 'error'){
            $entry_error = true;
        }
        ...
        $message = UI_ITEM_ENTRY_TEMPLATE . "が選択されていません";
        $entry_error = true;
        createLogs(LOG_TYPE_ERROR, UI_ITEM_ENTRY_TEMPLATE . "有効化 - " . UI_ITEM_ENTRY_TEMPLATE . "未選択");
    }
}
...
if (isset($_POST['off_btn'])) {
    ...
    if (isset($_POST['selected_datas'])) {
        $result_data = entryTemplatesPostActionForDeactivateEvent($_SESSION['user_groups_id'],$_POST['selected_datas']);
        $result = json_decode($result_data,True);
        if($result['status'] == 'error'){
            $entry_error = true;
        }
        ...
        $message = UI_ITEM_ENTRY_TEMPLATE . "が選択されていません";
        $entry_error = true;
        createLogs(LOG_TYPE_ERROR, UI_ITEM_ENTRY_TEMPLATE . "無効化 - " . UI_ITEM_ENTRY_TEMPLATE . "未選択");
    }
}
...
if (isset($_POST['delete_btn'])) {
    ...
    if (isset($_POST['selected_datas'])) {
        $result_data = entryTemplatesPostActionForDeleteEvent($_SESSION['user_groups_id'],$_POST['selected_datas']);
        $result = json_decode($result_data,True);
        if($result['status'] == 'error'){
            $entry_error = true;
        }
        ...
        $message = UI_ITEM_ENTRY_TEMPLATE . "が選択されていません";
        $entry_error = true;
        createLogs(LOG_TYPE_ERROR, UI_ITEM_ENTRY_TEMPLATE . "削除 - " . UI_ITEM_ENTRY_TEMPLATE . "未選択");
    }
}

// 無効化処理
$auto_deactivate_result_data = autoDeactivateForEntryTemplate($_SESSION['user_groups_id']);
...
// 有効化処理
$auto_activate_result_data = autoActivateForEntryTemplate($_SESSION['user_groups_id']);
...
$template_data = getEntryTemplateList($_SESSION['user_groups_id']);
    ...
    <div class="btn-area">
        <div class="btn-center-area">
            <div class='btn-item'><a class='link-btn'
                                     href='/Interfaces/Views/Pages/user/group/setting/template/entry_template/user_group_entry_template_entry.php'><?php echo UI_ITEM_ENTRY_TEMPLATE; ?>登録</a>
            </div>
        </div>
    </div>
        ...
        <div class="btn-right-area">
            ...
            <div class="btn-item">
                <button type="submit" class="btn btn-primary" name="use_btn"
                        onclick="return confirm('本当に実行しますか？\n以下の<?php echo UI_ITEM_ENTRY_TEMPLATE; ?>はスキップされます。\n・ 既に使用状態の<?php echo UI_ITEM_ENTRY_TEMPLATE; ?>\n・ 無効状態の<?php echo UI_ITEM_ENTRY_TEMPLATE; ?>')">
                    使用する
                </button>
            </div>
            <div class="btn-item">
                <button type="submit" class="btn btn-warning" name="un_use_btn"
                        onclick="return confirm('本当に実行しますか？\n以下の<?php echo UI_ITEM_ENTRY_TEMPLATE; ?>はスキップされます。\n・ 既に未使用状態の<?php echo UI_ITEM_ENTRY_TEMPLATE; ?>\n・ 無効状態の<?php echo UI_ITEM_ENTRY_TEMPLATE; ?>')">
                    使用しない
                </button>
            </div>
            <div class="btn-item">
                <button type="submit" class="btn btn-primary" name="on_btn"
                        onclick="return confirm('本当に実行しますか？\n以下の<?php echo UI_ITEM_ENTRY_TEMPLATE; ?>はスキップされます。\n・ 既に有効化の<?php echo UI_ITEM_ENTRY_TEMPLATE; ?>')">
                    有効化
                </button>
            </div>
            <div class="btn-item">
                <button type="submit" class="btn btn-warning" name="off_btn"
                        onclick="return confirm('本当に実行しますか？\n以下の<?php echo UI_ITEM_ENTRY_TEMPLATE; ?>はスキップされます。\n・ 既に無効化の<?php echo UI_ITEM_ENTRY_TEMPLATE; ?>')">
                    無効化
                </button>
            </div>
            <div class="btn-item">
                <button type="submit" class="btn btn-danger" name="delete_btn"
                        onclick="return confirm('本当に実行しますか？\n以下の<?php echo UI_ITEM_ENTRY_TEMPLATE; ?>はスキップされます。\n・ 有効化の<?php echo UI_ITEM_ENTRY_TEMPLATE; ?>')">
                    削除
                </button>
            </div>
        </div>
        ...
        <div class="table-area">
            <table class="table table-light table-striped table-bordered table-hover">
                ...
                <tr>
                    ...
                    <th><?php echo UI_ITEM_ENTRY_TEMPLATE_USE; ?></th>
                    <th><?php echo UI_ITEM_ENTRY_TEMPLATE_STATUS; ?></th>
                </tr>
                </thead>
...
'
               style='pointer-events: none;'></td>";
                    ...
                    echo "
                <td><a href='/Interfaces/Views/Pages/user/group/setting/template/entry_template/user_group_entry_template_update.php?template_id={$template_id}'>$template_id</a></td>
                ";
