...
if (isset($_POST['user_assign'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['selected_users'])) {
        ...
        foreach ($selected_users as $user) {
            ...
            apiCallGroupInfoCreate($groups_id, $post_user_id, 0);
            $suc_cnt += 1;
        }
                ...
                <div class="settings">
                    ...
                    <div class="settings-section">
                        ...
                        <div class="settings-form">
                            ...
                            <div class="settings-input-container">
                                <input disabled type="text" minlength="4" maxlength="32" class="form-control"
                                       name="group_name"
                                    <?php
        ...
        <div class="table-area">
            <table class="table table-light table-striped table-bordered table-hover">
                ...
                foreach ($users_data
                    ...
                    if ($permission_value != 0 or $approval_value == 0 or $deleted_value == 1 or $groups_name != "-") {
                        $class = "class='lock-rows'";
                        $onclick = ""; // クリックなし
                        $is_hidden = "hidden";
                    } else {
                        $class = "class='clickable-row'";
                        $onclick = " onclick=\"window.location='/Interfaces/Views/Pages/admin/admin_user_update.php?screen=group_assign&user_id={$user_id}&groups_id={$groups_id}&page={$page}'\"";
                        $is_hidden = "";
                    }

                    echo "<tr $class $onclick style='cursor: pointer;'>";
                    echo "<td onclick='event.stopPropagation(); toggleCheckbox(this)'>";

                    if ($is_hidden != "hidden") {
                        echo "<input type='checkbox'
                               name='selected_users[]'
                               value='{$user_id}\t{$permission_value}\t{$approval_value}\t{$deleted_value}\t{$group_leader}\t{$groups_name}'
