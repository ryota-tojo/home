...
class ShoppingEntryTemplateServiceTest extends Specification {
    ...
    def "shoppingEntryTemplate_refer_#useCase"() {
        ...
        "正常_データなし" | new ShoppingEntryTemplateReferResult(ResponseCode.データ不在エラー.code, null) | null
    }
    ...
    def "shoppingEntryTemplate_save_#useCase"() {
        ...
        setup:
        def groupsId = FixtureGroupList.所属グループID_正常()
        def templateNo = FixtureShoppingEntryTemplate.テンプレートNO_正常()
        ...
        saveCnt * shoppingEntryTemplateRepository.save(groupsId, templateNo, templateId, templateName, memberId, categoryId, type, payment, settlement, amount, remarks, UseFlag) >> template
        result == expected
    ...
    def "shoppingEntryTemplate_update_#useCase"() {
        ...
        choiceCnt2 * choicesRepository.getItemName(FixtureChoices.選択肢_支払い方法(), _) >> masterCoices2
        choiceCnt3 * choicesRepository.getItemName(FixtureChoices.選択肢_精算状況(), _) >> masterCoices3
        updateCnt * shoppingEntryTemplateRepository.update(groupsId, templateNo, templateId, templateName, memberId, categoryId, type, payment, settlement, amount, remarks, UseFlag) >> updateRows
        result == expected
    ...
    def "shoppingSearchTemplate_usage_#useCase"() {
        ...
        then:
        1 * shoppingEntryTemplateRepository.usage(groupsId, templateId) >> updateRows
        result == expected

        where:
        useCase           | expected                                                         | updateRows
        "正常"            | new ShoppingEntryTemplateUpdateResult(ResponseCode.成功.code, 1) | 1
        "正常_データなし" | new ShoppingEntryTemplateUpdateResult(ResponseCode.成功.code, 0) | 0
    }

    def "shoppingSearchTemplate_unUsage_#useCase"() {
    ...
    def "shoppingEntryTemplate_setDeleted_#useCase"() {
        ...
        then:
        1 * shoppingEntryTemplateRepository.setDeleted(groupsId, templateId) >> updateRows
        result == expected

        where:
        useCase                 | expected                                                                     | updateRows
        "正常"                  | new ShoppingEntryTemplateUpdateResult(ResponseCode.成功.code, 1)             | 1
        "異常_データ不在エラー" | new ShoppingEntryTemplateUpdateResult(ResponseCode.データ不在エラー.code, 0) | 0
    }
